...
class ApprovalRequest(models.Model):
    ...
    timesheet_id = fields.Many2one('hr.timesheet.report', string='Timesheet', required=False)
        ...
        if approver_count == 0:
            ...
            if tot_refused_count == tot_approver_count:
                if self.timesheet_id:
                    self.timesheet_id.sudo().action_refuse()
    ...
    def action_approve(self, approver=None):
        ...
        if approver_count == 0:
            ...
            if tot_approver_count == tot_approved_count:
                if self.timesheet_id:
                    self.timesheet_id.sudo().action_approve()
        return res
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    if request.timesheet_id:
                        request.timesheet_id.sudo().action_approve()

                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.timesheet_id:
                        request.timesheet_id.sudo().action_refuse()
                else:
                    status = 'pending'
            else:
                status = 'new'
...
class ApprovalApprover(models.Model):
    ...
    timesheet_id = fields.Many2one(related='request_id.timesheet_id')