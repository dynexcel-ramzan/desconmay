...
class ApprovalRequest(models.Model):
    ...
    site_ot_id = fields.Many2one('hr.overtime.approval', string='Overtime', required=False)
    ...
    def action_refuse(self, approver=None):
        ...
        if approver_count == 0:
            ...
            if tot_refused_count ==  tot_approver_count:
                if self.site_ot_id:
                    self.site_ot_id.sudo().action_refuse()        
    ...
    def action_approve(self, approver=None):      
        ...
        if approver_count == 0:
            ...
            if tot_approver_count ==  tot_approved_count: 
                if self.site_ot_id:
                    self.site_ot_id.sudo().action_approved()       
        ...
        for request in self:
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    status = 'approved'
                    if request.site_ot_id:
                        request.site_ot_id.sudo().action_approve()
                    
                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.site_ot_id:
                        request.site_ot_id.sudo().action_refuse()    
                else:
                    status = 'pending'
            else:
                status = 'new'
...
class ApprovalApprover(models.Model):
    ...
    site_ot_id = fields.Many2one(related='request_id.site_ot_id')